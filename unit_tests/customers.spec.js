...
describe('POST /customer from login form', () => {
    it('GET /customers Retrieves user information if present in db', async () => {
        const res = await request.post('/api/customer/test123').send(
            {password: testPassword, email: 'joe@email.com'}
        )
        expect(res.status).toBe(200)
    ...
    it('GET /customers Returns not found if user information is not present in db', async () => {
        const res = await request.post('/api/customer/test12345').send(
            {password: testPassword, email: 'joes@email.com'}
        )
